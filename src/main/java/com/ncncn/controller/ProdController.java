...
public class ProdController {
	...
	public void prodList(GiftiCriteria cri, Model model){
		...
		model.addAttribute("category", service.getCate(code)); 			// 카테고리
		model.addAttribute("brandList", service.getBrandList(code)); 	// 브랜드 목록
		model.addAttribute("gifti", service.getGiftiWithPaging(cri));	// 기프티콘 목록(페이징 처리 포함)
	...
	public void prodDetail(HttpServletRequest request, GiftiCriteria cri, WishListVO wish, Model model){
		...
		model.addAttribute("category", service.getCate(code));			// 카테고리
		model.addAttribute("brandList", service.getBrandList(code));		// 브랜드 목록
		model.addAttribute("giftiList", service.getGiftiList(code));		// 등록된 기프티콘 목록
		model.addAttribute("pageMaker", new PageDTOHY(cri, total)); 		// 카테고리바에서 prod_list로 이동시 필요
		model.addAttribute("gifticon", service.getGifti(code));			// 대표 기프티콘
